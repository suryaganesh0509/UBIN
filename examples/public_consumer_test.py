#!/usr/bin/env python3
"""
UBIN v1.0.3 public-consumer integration test.
...
Right now, before the PyPI release exists, install the tagged GitHub release:
    python3 -m pip install "git+https://github.com/suryaganesh0509/UBIN.git@v1.0.3"

Optional one-command bootstrap:
    python3 examples/public_consumer_test.py --install

`--install` installs the exact v1.0.3 GitHub tag only when UBIN is missing.
...
114-case pytest suite when the dev dependencies are installed.
"""
...
GITHUB_V1 = (
    "git+https://github.com/suryaganesh0509/UBIN.git@v1.0.3"
)
...
def load_ubin(auto_install: bool):
    try:
        ...
        if not auto_install:
            ...
            print("Install v1.0.3 from GitHub:")
            print(
        ...
        print("UBIN is missing. Installing tagged UBIN v1.0.3 from GitHub...")
...
parser = argparse.ArgumentParser(
    description="Run UBIN v1.0.3 public-consumer integration tests."
)
parser.add_argument(
    "--install",
    action="store_true",
    help="Install the tagged UBIN v1.0.3 GitHub release if UBIN is missing.",
)
...
print("=" * 72)
print("UBIN v1.0.3 PUBLIC-CONSUMER INTEGRATION TEST")
print("=" * 72)
...
print("ALL PUBLIC-CONSUMER UBIN v1.0.3 INTEGRATION CHECKS PASSED.")
