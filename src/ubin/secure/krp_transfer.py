...
from .session import derive_permutation_key, derive_transfer_key
from .krp import frame_context, permute_blocks, restore_blocks
...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise UbinResumeError(f"invalid UBIN KRP resume state: {path}") from exc
...
def _ticket_payload(header: KrpTransferHeader) -> bytes:
    name = header.filename.encode("utf-8")
    return (
        b"UBIN-v0.5-krp-resume-ticket\0"
        + header.transfer_id
...
def _make_ticket(server, header: KrpTransferHeader) -> bytes:
...
def _partial_path(server, header: KrpTransferHeader) -> Path:
    return (
...
def _stable_header_dict(header: KrpTransferHeader) -> dict[str, Any]:
    return {
        "version": 2,
        "layout": "krp",
    ...
    *,
    ...
    with UbinObject(source) as obj:
        ...
        if state_path.exists():
            state = _read_json(state_path)
            if state.get("version") != 2 or state.get("layout") != "krp":
                raise UbinResumeError("resume state is not a UBIN v0.5 KRP state")
            if state.get("source_path") != str(source_path.resolve()):
                raise UbinResumeError("KRP resume state belongs to another source")
            ...
            try:
                ...
                raise UbinResumeError("invalid client KRP resume state") from exc
        ...
        try:
            with tls_sock:
                ...
                permutation_key = derive_permutation_key(
                    session_key,
                    transfer_id,
                )
                ...
                if status_magic == ERROR_MAGIC:
                    ...
                    raise UbinNetworkError(
                        f"server rejected KRP resumable transfer: {message}"
                    )
                ...
                for frame_number in range(next_frame, frame_count):
                    ...
                    wire_ciphertext = permute_blocks(
                        ciphertext,
                        permutation_key,
                    ...
                    tls_sock.sendall(wire_ciphertext)
...
def receive_krp_after_magic(
    ...
    header = KrpTransferHeader.unpack(fixed, filename_bytes)
    ...
    permutation_key = derive_permutation_key(session_key, header.transfer_id)
    cipher = AESGCM(transfer_key)
    ...
    with partial_path.open("r+b", buffering=0) as out:
        ...
        for expected_frame in range(next_frame, header.frame_count):
            ...
            wire_ciphertext = _recv_exact(tls_sock, ciphertext_len)
            ciphertext = restore_blocks(
                wire_ciphertext,
                permutation_key,
                context=frame_context(
                    header.transfer_id,
