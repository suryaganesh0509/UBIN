...
def _page(body: str, title="UBIN v1.0.0 Demo") -> bytes:
    return f'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{html.escape(title)}</title><style>{CSS}</style></head><body><div class="wrap"><div class="brand">UBIN <span class="ver">v1.0.0</span></div>{body}<div class="footer">UBIN handles the bytes. You handle the logic. • Local demo only</div></div></body></html>'''.encode()
